...
class BrowserView(QWebEngineView):
    ...
    def _inject_offline_check(self):
        ...
        # Escape backticks within the HTML content for JavaScript template literal
        escaped_offline_page_content = OFFLINE_PAGE_TPL.replace('`', '\\`')
        script_code = OFFLINE_CHECK_SCRIPT_TPL.replace(
            '<!-- OFFLINE_PAGE_PLACEHOLDER -->', escaped_offline_page_content
        )
        run_script(self, script_code, name="offlineCheck")
        # No need to runJavaScript here, run_script handles injection point DocumentReady
